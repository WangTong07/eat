...
export default function OverviewCards() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setWeekExpense((exp ?? []).reduce((sum: number, e: any) => sum + Number(e.amount || 0), 0));
        ...
        <Link href="/finance" className="ui-card rounded-xl p-5 card-hover animate-slide-up block" style={{ animationDelay: "0.4s" }}>
          <div className="flex items-start justify-between">
            <div className="w-full">
              <p className="text-muted text-sm">本周预算使用</p>
              <h3 className="text-xl font-bold mt-1 text-heading">¥{weekExpense.toFixed(0)} / ¥—</h3>
              <div className="mt-3">
