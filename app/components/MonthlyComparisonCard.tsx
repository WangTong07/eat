...
export default function MonthlyComparisonCard({ currentMonth, currentAmount }: MonthlyComparisonCardProps) {
  ...
  const stats = useMemo(() => {
    ...
    const lastMonth = monthlyData[monthlyData.length - 2]; // 上个月
    const currentMonthData = monthlyData[monthlyData.length - 1]; // 当前月
    
    // 更新当前月的实际金额
    const updatedCurrentAmount = currentAmount;
    const lastMonthAmount = lastMonth?.amount || 0;
    
    // 计算环比变化
    const changeAmount = updatedCurrentAmount - lastMonthAmount;
    const changePercent = lastMonthAmount > 0 ? (changeAmount / lastMonthAmount) * 100 : 0;
    
    // 计算最大值用于图表缩放
    const maxAmount = Math.max(...amounts, updatedCurrentAmount);
      ...
      <div className="space-y-2 mb-4">
        <div className="flex items-baseline gap-2">
          <div className="text-4xl font-bold bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">
            ¥{currentAmount.toLocaleString('zh-CN', { minimumFractionDigits: 0 })}
          </div>
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-end justify-between gap-1 h-16 bg-gray-800/30 rounded-lg p-2">
          {monthlyData.map((data, index) => {
            ...
            const amount = isCurrentMonth ? currentAmount : data.amount;
              ...
              <div key={data.month} className="flex flex-col items-center gap-1 flex-1">
                <div 
                  className={`w-full rounded-sm transition-all duration-300 ${
                    isCurrentMonth 
                      ? 'bg-gradient-to-t from-purple-500 to-pink-500 shadow-md' 
                ...
                <div className={`text-xs font-medium ${
                  isCurrentMonth ? 'text-purple-300' : 'text-gray-500'
                }`}>
                  {data.displayName}
                </div>
              </div>
