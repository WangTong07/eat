...
function classifyByRules(name: string): { category: string; confidence: number } {
  ...
  // 豆制品 - 高置信度（专门处理豆腐等豆制品）
  if (/(豆腐|豆干|豆皮|豆腐皮|千张|腐竹|豆腐丝|臭豆腐|嫩豆腐|老豆腐|内酯豆腐|豆腐块|豆腐条|豆制品)/.test(name)) {
    return { category: "蔬果类", confidence: 0.95 };
  }
  ...
  // 其他蔬菜 - 低置信度（移除豆腐，因为已经在上面专门处理）
  if (/(菜|葱|姜|蒜|豆芽|藕|金针菇|香菇|蘑菇|菌|茄子|青菜|油麦菜|娃娃菜|生菜|菠菜|西兰花|花菜|空心菜|豆角|四季豆|芹菜|黄瓜|冬瓜|南瓜|苦瓜|海带|木耳|莴笋|莴苣|莲藕|韭菜|蒜薹|香菜|花生|核桃|杏仁)/.test(name)) {
